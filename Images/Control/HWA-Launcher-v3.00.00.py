...
import ctypes  # <-- Added to handle Windows API calls
...
def enforce_single_instance():
    ...
    try:
        ...
        temp_root = tk.Tk()
        temp_root.withdraw()
        temp_root.update()  # <-- ADD THIS LINE to cleanly wipe it from the taskbar tracking loop
...
class LauncherApp:
    def __init__(self, root):
        self.root = root
        self.root.title("HWA-Launcher-v3.00.00")
        ...
        self.theme_colors = {}
        self.transparent_color = "#000001" # Reserved for the Layered Window Hack
        ...
        self.help_btn = tk.Button(self.controls_frame, text="❓ Help", command=self.show_help, relief="flat", padx=rem(0.5))
        self.help_btn.pack(side="left", padx=rem(0.2))

        self.refresh_btn = tk.Button(self.controls_frame, text="🔄 Refresh", command=self.refresh_buttons, relief="flat", padx=rem(0.5))
        self.refresh_btn.pack(side="left", padx=rem(0.2))

        self.edit_btn = tk.Button(self.controls_frame, text="📝 Edit Paths", command=self.edit_paths_file, relief="flat", padx=rem(0.5))
        self.edit_btn.pack(side="left", padx=rem(0.2))

        # --- SUBHEADER UI (THEME & OPACITY) ---
        self.subheader_frame = tk.Frame(root)
        self.subheader_frame.pack(fill="x", padx=rem(1), pady=(0, rem(0.5)))
        
        self.theme_btn = tk.Button(self.subheader_frame, text="☀️ Light Mode", command=self.toggle_theme, relief="flat", padx=rem(0.5))
        self.theme_btn.pack(side="left")

        self.opacity_wrap = tk.Frame(self.subheader_frame)
        self.opacity_wrap.pack(side="right", fill="x", expand=True, padx=(rem(1), 0))
        ...
        # Fixed length slider to prevent resizing bugs
        ...
        self.disclaimer_lbl = tk.Label(self.footer_frame, text=disclaimer_text, font=("Segoe UI", 8), pady=rem(0.3))
        self.disclaimer_lbl.pack()

        # --- LAYERED WINDOW HACK FOR ISOLATED TRANSPARENCY ---
        # 1. We create a transparent 'hole' frame in the main window
        self.hole_frame = tk.Frame(root, bg=self.transparent_color, bd=0, highlightthickness=0)
        self.hole_frame.pack(fill="both", expand=True, padx=rem(1), pady=rem(0.5))

        if self.use_layered_hack:
            self.root.wm_attributes("-transparentcolor", self.transparent_color)
            
            # --- FIXED DUAL ICON ISSUE ---
            # Pass self.root as the master layout parameter so Windows embeds it under one taskbar icon
            self.list_window = tk.Toplevel(self.root) 
            # Force the OS to treat this window as a child dialog of the main root window
            self.list_window.transient(self.root)
            
            self.list_window.withdraw() # Hide immediately to prevent black flash
            self.list_window.overrideredirect(True)
            
            # The canvas and scrollbar go inside this floating window
            self.list_container = tk.Frame(self.list_window, bd=0, highlightthickness=0)
            self.list_container.pack(fill="both", expand=True)
            
            # Real-time synchronization bounds
            self.root.bind("<Configure>", self.sync_windows)
            self.hole_frame.bind("<Configure>", self.sync_windows)
            
            # Z-Order synchronization (Fixes the black screen / behind-window issue)
            self.root.bind("<Map>", self._on_root_map)
            self.root.bind("<Unmap>", self._on_root_unmap)
            self.root.bind("<FocusIn>", self._on_root_focus)
        else:
            # Safe Fallback for macOS/Linux (Applies opacity to whole window instead)
        ...
        # --- FIX: DISABLE MAXIMIZE & EDGE SNAPPING ---
    ...
    def _disable_maximize_and_snap(self):
        ...
        try:
            ...
            # Grab the raw HWND of the Tkinter frame
            hwnd = int(self.root.wm_frame(), 16)
            GWL_STYLE = -16
            WS_MAXIMIZEBOX = 0x00010000
            
            # Fetch current style, strip WS_MAXIMIZEBOX, and apply
    ...
    def _on_root_focus(self, event):
        if self.use_layered_hack and self.list_window.state() == "normal":
            self.list_window.lift()

    def sync_windows(self, event=None):
        """Snaps the floating borderless window exactly over the transparent hole in real-time."""
        if not self.use_layered_hack: return
        if not self.root.winfo_viewable(): return
        
        # Prevent infinite Configure loops by ignoring non-target events
    ...
    def apply_theme(self, resize=True):
        if self.is_dark_mode:
            self.theme_colors = {
                "bg_main": "#1e1e1e", "bg_list": "#1e1e1e", "bg_foot": "#141414",
            ...
            self.theme_colors = {
                "bg_main": "#f0f0f0", "bg_list": "#ffffff", "bg_foot": "#e0e0e0",
        ...
        self.footer_frame.configure(bg=tc["bg_foot"])
        self.disclaimer_lbl.configure(bg=tc["bg_foot"], fg=tc["fg_mut"])
        ...
        self.help_btn.configure(bg=tc["bg_btn"], fg=tc["fg_main"], activebackground=tc["bg_btn_act"], activeforeground=tc["fg_btn"])
        self.refresh_btn.configure(bg=tc["bg_btn"], fg=tc["fg_main"], activebackground=tc["bg_btn_act"], activeforeground=tc["fg_btn"])
        self.edit_btn.configure(bg=tc["bg_btn"], fg=tc["fg_main"], activebackground=tc["bg_btn_act"], activeforeground=tc["fg_btn"])
    ...
    def auto_adjust_window(self):
        ...
        target_h = head_h + sub_h + foot_h + list_h + rem(3.0) 
    ...
    def refresh_buttons(self, resize=True):
        ...
        for index, (path, label) in enumerate(valid_paths):
            ...
            # Formatting Display Name and Colors based on Theme
            if is_web:
    ...
    def launch(self, path):
        # SECURITY AUDIT: Validate inputs natively to prevent arbitrary script injections
    ...
    def show_help(self):
        ...
        help_w = rem(25)  
        help_h = rem(38)  # Adjusted height properly to guarantee full text visibility without cutoff
